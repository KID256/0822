...
#define DEVICE_NAME "motion_detector"
#define SENSOR_GPIO 17
#define LED_GPIO 18
#define LED_ON_TIME_MS 700  // LED on time in milliseconds

static int major_number;
static struct class *motion_detector_class;
static struct device *motion_detector_device;
...
static int __init motion_detector_init(void)
{
    int ret;
    ...
    // Create device class
    motion_detector_class = class_create(THIS_MODULE, DEVICE_NAME);
    if (IS_ERR(motion_detector_class)) {
        pr_err("Failed to create device class\n");
        unregister_chrdev(major_number, DEVICE_NAME);
        return PTR_ERR(motion_detector_class);
    }

    // Create device
    motion_detector_device = device_create(motion_detector_class, NULL, MKDEV(major_number, 0), NULL, DEVICE_NAME);
    if (IS_ERR(motion_detector_device)) {
        pr_err("Failed to create the device\n");
        class_destroy(motion_detector_class);
        unregister_chrdev(major_number, DEVICE_NAME);
        return PTR_ERR(motion_detector_device);
    }
    ...
    // Request IRQ
    ret = request_irq(gpio_to_irq(SENSOR_GPIO), motion_interrupt, IRQF_TRIGGER_RISING, "motion_detector", NULL);
    if (ret) {
        pr_err("Unable to request IRQ\n");
        device_destroy(motion_detector_class, MKDEV(major_number, 0));
        class_destroy(motion_detector_class);
        unregister_chrdev(major_number, DEVICE_NAME);
        return ret;
    }

    pr_info("Motion detector module initialized\n");
    return 0;
}

static void __exit motion_detector_exit(void)
{
    ...
    class_destroy(motion_detector_class);
    unregister_chrdev(major_number, DEVICE_NAME);
    pr_info("Motion detector module unloaded\n");
}

module_init(motion_detector_init);
module_exit(motion_detector_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Your Actual Name");
MODULE_DESCRIPTION("A simple motion detector kernel module with real-time signaling");
MODULE_VERSION("1.2");
